...
#%%
# Loop data acquisition
for x in range(checkpoint_val, 2000, 100):
  ...
  chirp_req = "SELECT TOP 200 C.Batch, C.Chip, C.TestTemp, C.RFTestFreq, C.PredictChannel, C.SMSR, C.Chirp, C.LISlope, C.Timestamp \
  ...
  chirp_df = pd.DataFrame(data=chirp_res, columns=['Batch', 'Chip', 'TestTemp', 'RFTestFreq', 'PredictChannel', 'SMSR', 'Chirp', 'LISlope', 'Timestamp'] )

  connected_df = pd.merge(li_df, chirp_df, on=['Batch', 'Chip'])
  COB_data = COB_data.append(connected_df, ignore_index=True)
    
  checkpoint['checkpoint'] = x+100
  checkpoint_val = x+100
